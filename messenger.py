...
def main():
    ...
    moderate_rainy_hours = ", ".join([hr[11:16] for hr in filter_dates_by_amount(read_hourly_dataframe[:24], THRESHOLD_AMOUNT,HIGH_RAINFALL)])
    high_rainy_hours = ", ".join([hr[11:16] for hr in filter_dates_by_amount(read_hourly_dataframe[:24], HIGH_RAINFALL, 100)])

    print(moderate_rainy_hours)
    print(high_rainy_hours)

    if high_rainy_hours and moderate_rainy_hours:
    ...
    elif moderate_rainy_hours and not high_rainy_hours:
        message = f"Moderate chance of rainfall is expected at hours {moderate_rainy_hours}"
    elif high_rainy_hours and not moderate_rainy_hours:
        message = f"High chance of rainfall is expected at {high_rainy_hours}"
    else:
